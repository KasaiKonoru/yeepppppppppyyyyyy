    ...
    for pc in priv_channels:
        ...
        if 'request_time' in pcv and time() - pcv['request_time'] > 120:
            ...
            await pcv['text_channel'].send(
                ...
                "Your channel is still locked, but there's now no way for anyone to join you. "
                "Use `{}unlock` to make it public again."
...
class MyClient(discord.AutoShardedClient):
    ...
    async def on_ready(self):
        ...
        print('=' * 24)
        nc=self.user.name
        await func.admin_log("**Logged in.**\n▬▬▬▬▬▬", self)
        activity = discord.Activity(name='vc/help', type=discord.ActivityType.streaming, url="https://twitch.tv/20secondsviral")
            ...
            if msg_channel and creator and vc:
                try:
                    m = await msg_channel.send(
                        "Hey {},\n{} would like to join your locked voice channel. React with:\n"
                        "• ✅ to **allow**.\n"
                        "• ❌ to **deny** this time.\n"
